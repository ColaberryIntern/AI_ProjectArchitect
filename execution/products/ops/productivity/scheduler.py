...
Fires every weekday morning (07:30 ET by default) and runs runner.run(), which
writes the HTML to disk and, when PRODUCTIVITY_REPORT_DELIVERY=1, emails it to
the recipients in config/report_recipients.json.
...
Schedule is overridable via env for prod tuning:
    PRODUCTIVITY_REPORT_HOUR   (default 7)
...
def start_scheduler() -> BackgroundScheduler | None:
    ...
    hour = int(os.environ.get("PRODUCTIVITY_REPORT_HOUR", "7"))
    ...
    _scheduler = sched
    logger.info(
        "productivity report scheduler started — %s %02d:%02d ET, delivery_enabled=%s",
        dow, hour, minute, os.environ.get("PRODUCTIVITY_REPORT_DELIVERY", "0") == "1",
    )
    return sched
