...
PERSONAS: list[dict] = [
    ...
    },
    {
        ...
        "label": "Visual-first",
        "emoji": "📊",
        "blurb": "An executive dashboard opens in your browser — KPI cards, charts & diagrams picked from your data, decisions and Basecamp actions pre-filled to paste back. Dyslexia-friendly.",
        "working_block": (
            "## How I want you to work\n"
            "I think visually and want to SEE the story in this work and review a decision, not read dense "
            "prose or retype anything. Do NOT answer in dense prose, and do NOT make any Basecamp or code "
            "change yet. Build me ONE self-contained, professional, executive-grade HTML sheet (inline "
            "CSS+JS, no external dependencies except the Mermaid and Chart.js CDNs) driven by a single "
            "embedded data object, and OPEN IT IN MY BROWSER automatically. I review it, adjust only what's "
            "wrong, then paste the generated prompt back to you to execute everything at once.\n"
            "1. LOOK — executive, NOT playful: a clean business one-pager, not a kids' app. Slate page "
            "background (#eef2f6), white cards with thin #e2e8f0 borders and a soft shadow, ink headings "
            "(#0f172a), and ONE restrained accent (teal #0f766e by default — never a rainbow). Use a muted, "
            "SEMANTIC status palette for conditional color and keep it consistent: green (#067647 on "
            "#ecfdf3) = good / done / covered, amber (#b54708 on #fffaeb) = warning / medium, red (#b42318 "
            "on #fef3f2) = risk / gap, slate = neutral; and tie each category to ONE color everywhere it "
            "appears (a phase keeps its color in the chart, the timeline AND the matrix). Segoe UI / system "
            "font, generous spacing, large readable text, a sticky top nav. No neon, no bright gradients, no "
            ...
            "you need to do:** the action in one line. Lead with the conclusion and make the single most "
            "important number or finding impossible to miss. This brief is the first thing I read.\n"
            "3. TELL THE STORY WITH VISUALS — model the data FIRST into ONE embedded JS object that BOTH the "
            "tables and the charts read (never hand-duplicate numbers), then CHOOSE the visuals from the "
            "SHAPE of the data, not a fixed template: headline numbers / counts → KPI cards at the top "
            "(max 6); category counts per group → a Chart.js bar or doughnut; before/after or A-vs-B → a "
            "Chart.js grouped bar; time / schedule / phases → a Mermaid gantt (or a Chart.js line for "
            "trends); process / dependency / flow → a Mermaid flowchart; step-by-step interaction → a "
            "Mermaid sequence diagram; coverage / many-to-many / matrix → a conditional-color heatmap table "
            "whose cells tint by value so outliers jump out; full records → detail cards or a sortable "
            "table that shows every row, never summarized away. Use only the visuals the data justifies, "
            "give every chart a one-line 'so what' caption, and let everything drill from headline → "
            "detail.\n"
            "4. DECISIONS — keep them FEW. Only surface choices that genuinely change the outcome; decide "
            ...
            "5. BASECAMP ACTIONS — decide for me, don't interrogate me. Show a 'Basecamp actions' panel of "
            ...
            "6. ROUND-TRIP — put a 'Copy Claude Code prompt' button at BOTH the TOP and the BOTTOM, with a "
            ...
            "7. BUILD IT TO RUN — one .html file, no build step; load Mermaid and Chart.js from CDN, vanilla "
            "JS only, and render every card, table, and chart from the single embedded data object. "
            "Initialize Mermaid with a calm 'base' theme and set Chart.js defaults to the palette. CDN libs "
            "need internet — offer to inline them for an offline version.\n"
            "8. If a Stop condition triggers, show it as a clear callout at the top of the sheet, then still "
            "build the rest."
        ),
    },
    {
