...
- **`connection_identity_suspect: true` (or a `connection_identity_suspect` ring-buffer entry).** The wrong-account self-annealing guard fired: the token authenticates as one BC numeric id but the classifier expects a *different* id, and projects 403'd. That combination means the operator's BC OAuth grant is bound to the **wrong / a duplicate Basecamp person record** for their email (the 2026-06-10 incident: Ali connected as BC `16988292`, a member of nothing, while his tasks live under `17454835`). The banner leads with a single root-cause line naming both ids; the fix is to **reconnect** (`/profile/connect-basecamp-ai`) as the correct account — *not* a per-project grant, which is a no-op against a non-member id. The guard is gated on 403s being present so it can never false-positive on the normal AI-clone model (token id ≠ classifier id by design, but a granted clone reads cleanly with zero forbidden buckets). Regression guards: `test_ops_sync.py::test_widespread_403_with_identity_mismatch_flags_connection_suspect` and `::test_identity_mismatch_without_403_is_not_flagged`.
