...
4. For each project: walk the dock → todoset → todolists → active + completed todos. **Every collection in this walk is paginated — `/projects.json`, `/todosets/{id}/todolists.json`, `/todolists/{id}/groups.json`, and `/todolists/{id}/todos.json` all go through `_paginate`, never a single-page `_bc_get`.** Basecamp returns todolists and groups in *position* order and **appends** newly-created ones to the end, so a fresh list/group lands on page 2+ once a project crosses one page. A single-page fetch silently skips everything past page 1, which makes a just-created list's tasks invisible while the old lists keep rendering — the **2026-06-18 incident** (Ali added a batch of lists/tasks to project 47126345 and My Day showed only the old tasks; root cause was the todolists and groups fetches being single-page while todos/projects already paginated). Regression guards: `test_ops_sync.py::test_paginates_todolists_so_new_lists_on_page_two_are_walked` and `::test_paginates_groups_within_a_list`.
...
6. For each todo, classify whether it belongs in this operator's queue (see *Classification* below). Drop anything not relevant.
7. Apply the freshness filter (see *Freshness window* below). Drop anything stale with no future due date.
8. Upsert the survivors into the local store (preserving operator-local fields — see *Store invariants*).
9. **Reconcile disappeared rows.** For each bucket walked without error, mark any locally-`active` row BC no longer returns in that bucket's active set as `completed`/`archived`, confirmed by a direct per-row GET (`sync._reconcile_walked_buckets`; see *Store invariants* #4). This catches completions the walk's best-effort completed-fetch missed, without waiting for the daily purge.
10. Persist `state.json` with `last_sync_at`, `last_sync_status`, `last_sync_error`, counts.
...
| `heartbeat(email) -> bool` | Renew a live slot's TTL clock to "now" for a still-PROGRESSING walk. The walk calls it after each todolist (`sync._walk_project_todos`), so the TTL means "no progress for TTL seconds" rather than "running for TTL seconds". Returns `True` iff a live slot was renewed; can't resurrect an already-expired one. |

**Per-todolist heartbeat (2026-06-22).** A mega-project — RMG Mortgage has **624 todolists**, Dev Team 269 — takes minutes to walk even with healthy BC (the `_walk_deadline` from the 522 fix only bounds *retries*, not a healthy walk's call volume). That can exceed the 300s TTL, at which point a second trigger would treat the healthy walk as crashed and start a wasteful **parallel** walk. The fix: `_walk_project_todos` pulses `coord.heartbeat(email)` after each todolist, and `_SyncLock.is_expired` measures the TTL from `last_progress_at` (falling back to `started_at`). So a long-but-progressing walk holds its slot indefinitely, while a genuinely crashed walk (heartbeats stop) is still reclaimed after the TTL. Both walk callers wire it: `pull_todos_for_project` and the full-sweep loop. Regression guards: `test_ops_sync_coordinator.py::TestHeartbeat` and `test_ops_sync.py::TestWalkHeartbeat`.

**TTL backstop.** A sync slot older than `LOCK_TTL_SECONDS_DEFAULT` (**300 seconds**, env `OPS_SYNC_LOCK_TTL_SECONDS`) is treated as crashed/abandoned and is silently replaced by the next caller. Without this, a SIGKILL'd worker would strand the operator indefinitely. Real syncs run ~30-60s, but a walk that retries through a Cloudflare 522 burst can legitimately run up to the per-project ceiling (`sync.PROJECT_SYNC_BUDGET_SECONDS`, ~180s). **The TTL MUST stay above that ceiling** — otherwise a long-but-healthy retry-extended walk is treated as crashed at the TTL and a *second* walk starts in parallel, piling load onto an already-struggling Basecamp (the failure mode the old 120s TTL would have hit during the 2026-06-18 522 storm). Regression guard: `test_ops_sync.py::TestSyncBudgetTtlInvariant::test_lock_ttl_exceeds_project_walk_ceiling`.

**Per-project walk ceiling (2026-06-18).** Each single project's walk is bounded by `PROJECT_SYNC_BUDGET_SECONDS` (180s) via a thread-local `_walk_deadline`: once a walk passes its deadline, `_bc_get` stops retrying transient 5xx/522 and fails fast, so a sustained BC outage can't stretch one large project to many minutes (the storm saw a 44-list project take ~13 min). The project keeps its last-good rows and converges on the next walk once BC recovers. The focused `pull_todos_for_project` walk uses the ceiling directly; the full sweep bounds each project to `min(ceiling, remaining overall budget)`.
...
**Past-month RETENTION (2026-06-22).** The freshness window gates **inclusion** (the walk's `_emit` won't add a stale todo), but the local store is a faithful mirror of Basecamp, so a row that was added while fresh and then went quiet stays `active` in the store as long as it's still active in BC. Without a matching retention rule, My Day showed those stale-but-active rows indefinitely — *more* than its "past month" promise (the 2026-06-22 finding: UX Team / Dev Team each had a task active in BC but untouched for 34–42 days still rendering). `sync.row_is_recent(row)` is the retention analogue of `_todo_is_relevant(todo)`, and `my_day._view_todos` applies it at **render time**: a row still `active` in BC but past the freshness window (no recent activity, no future due) is hidden from the view. This is a pure view-layer projection — the store is untouched (it must keep mirroring BC), and a stale row that gets fresh BC activity reappears on the next walk. This makes retention consistent with inclusion: a row the walk would refuse to add today is not shown today. Regression guards: `test_my_day_view_retention.py`.
...
- **BC API transient 5xx / Cloudflare 522 (2026-06-18).** Basecamp sits behind Cloudflare, which emits `520`–`524` edge errors (`522` = "origin connection timed out") and the occasional `5xx` in **bursts**. `_bc_get` retries every code in `TRANSIENT_HTTP_CODES` (`{500,502,503,504,520,521,522,524}`) — and bare connection errors (`URLError`/timeout) — up to `OPS_TRANSIENT_RETRIES` (3) times with exponential backoff (`OPS_TRANSIENT_BACKOFF_SECONDS`×2ⁿ). **Why it matters:** before this, a single 522 anywhere in a project's walk raised straight out of `_walk_project_todos`, so the project contributed **zero** rows, the upsert never touched it, and its local mirror froze. This was the **2026-06-18 stale-list incident**: project `47126345` was restructured to 17 todolists (old lists completed, 10 new lists holding all active work), but its walk almost always hit a 522 partway, so My Day kept rendering the *pre-restructure* lists no matter how many times Ali synced — the BC-vs-store diff showed all 10 new list ids missing from the store. A non-transient error (`401`/`403`) still propagates on the first call (no retry) so the membership-gap / identity guards fire correctly. Regression guards: `test_ops_sync.py::TestBcGet::test_522_retried_then_succeeds`, `::test_every_transient_code_is_retried`, `::test_403_is_not_retried`, `::test_500_transient_retried_then_raises`.
